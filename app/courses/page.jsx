...
import {
  ...
  Video, Users, LayoutGrid, Calendar, PlayCircle
} from "lucide-react";
...
export default function CourseManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const coursePayload = cRes.data.data;
      setCourses(coursePayload.courses || []);
      setStats({ total: coursePayload.totalCourses || 0 });

      // Handle User Data Structure (based on your JSON)
      const userPayload = uRes.data.data;
      setAllUsers(userPayload.users || []);
    } catch (err) {
  ...
  // Derived data based on roles in user array
  ...
  return (
    <ProtectedRoute allowedRoles={["ADMIN", "TEACHER"]}>
      <div className="space-y-6 text-slate-900 pb-12 p-4 md:p-8">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900">Course Catalog</h1>
            <p className="text-slate-500 text-sm font-medium">Manage multi-video assets and media.</p>
          </div>
          {isAdmin && (
            <button
              onClick={() => { setCurrentCourse(null); setIsModalOpen(true); }}
              className="bg-slate-900 text-white px-6 py-3 rounded-2xl flex items-center gap-2 hover:bg-slate-800 transition-all font-bold shadow-lg shadow-slate-200"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Active Courses</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50/50 p-4 rounded-[2rem] border border-slate-200">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-slate-900/5 text-sm font-medium"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <select
            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-bold cursor-pointer outline-none"
            value={teacherFilter}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  <tr><td colSpan={isAdmin ? 4 : 3} className="p-20 text-center animate-pulse font-bold text-slate-400">SYNCING WITH CLOUD...</td></tr>
                ) : filteredCourses.map((c) => (
                  <tr key={c._id} className="hover:bg-slate-50/30 transition-colors group">
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl border border-slate-100 overflow-hidden bg-slate-50 shrink-0">
                          {c.coursePic ? <img src={c.coursePic} className="w-full h-full object-cover" /> : <BookOpen className="m-auto mt-4 text-slate-200" />}
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 leading-tight">{c.title}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[10px] font-black uppercase text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md">{c.level}</span>
                    ...
                    <td className="p-6">
                      <div className="flex flex-col gap-1">
                        <span className="text-[11px] font-bold text-indigo-600 flex items-center gap-1">
                          <PlayCircle size={14} /> {c.videos?.length || 0} Lectures
                        </span>
                        {(c.videos?.length > 0) && (
                          <a
                            href={c.videos[0].videoUrl}
                            target="_blank"
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase hover:bg-indigo-100 transition-all w-fit"
                          >
                            <Video size={12} /> Watch Intro
                          </a>
                        )}
                      </div>
                    </td>
                ...
                {!loading && filteredCourses.length === 0 && (
                  <tr><td colSpan={4} className="p-20 text-center text-slate-400 font-medium">No courses found matching your criteria.</td></tr>
                )}
              </tbody>
            </table>
          </div>
